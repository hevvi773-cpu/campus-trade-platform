...
> 面向高校学生的半封闭社区 C2C 二手交易系统，Flask + MySQL 全栈实现。

## 🌐 线上地址（临时演示）

- **地址**：http://106.53.27.244（当前可访问，临时环境，不保证长期可用）
...
| 图表 | Chart.js（CDN） |
| 生产运行 | gunicorn + systemd（本地/服务器均可） |
...
| 文档 | 说明 |
|---|---|
| [PRD V1.5](docs/校园二手交易平台_PRD_V1.5.md) | 完整产品需求文档（产品 + 实现双视角），含用户画像/用户故事/竞品分析/指标体系/决策记录，11 张表 DDL、API 契约、隐私合规、回归清单、遗留问题 |
...
> ⚠️ **重要**：代码包中的 `.env` 包含开发环境配置（数据库密码、SECRET_KEY、管理员密码）。部署到可公开访问的服务器前请务必修改这些敏感信息，并不要将真实 `.env` 提交到公开仓库。
...
- **后续阶段**：独立完成十几次优化改造 + PRD 文档整理（实现对齐型，逆向整理 13 个历史版本）
