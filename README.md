...
本平台将分散的信息集中到统一入口，提供商品发布、审核、检索、下单、私信、评价、信用的完整交易闭环。
...
- 收藏（前端乐观更新 + 后端响应校正）
- 评价与信用体系（S/A/B/C/D 五级，评分公式驱动自动重算）
...
- 交易统计（已售 / 已买件数与金额）
- 数据可视化（近 6 个月月度交易柱状图、星级分布、全校信用等级分布）
- 我发布的商品管理（状态下拉筛选、删除）
- 评价记录（我发出的 / 我收到的）
...
| 指标 | 数值 |
|---|---|
| 注册账号 | 26 个（单人多账号全流程交叉验证） |
| 商品总数 | 51 件（待审核 2 / 已上架 47 / 已驳回 2） |
| 代码版本 | 15 个 |
| 路由数量 | 35 条 |
| 数据表 | 11 张 |
| 管理后台统计 SQL | 35 次 → 11 次 |
| 已跑通闭环 | 注册→发布→审核→下单→确认→评价→信用重算 |
